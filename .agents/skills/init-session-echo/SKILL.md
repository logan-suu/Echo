...
1. 检查以下 UI bootstrap 物化产物是否存在：
   ...
   - `.agents/skills/ui-bootstrap-build-echo/SKILL.md`
   - `.agents/skills/init-session-echo/SKILL.md`（当前文件）是否包含 Phase 3 UI 分支
   - `.agents/skills/next-task-echo/SKILL.md` 是否包含 Phase 3 UI handoff mode
2. 区分「文件已写入磁盘」与「命令已由当前 Codex 会话加载」——报告时明确标注。
