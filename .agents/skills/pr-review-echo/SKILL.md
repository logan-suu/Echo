...
1. **确定 PR 编号**：
   ...
   - 如果未提供，使用 `gh pr list --head [branch] --json number,url,title` 检查当前分支对应的 PR。
   - 如果有 PR，自动使用该编号。
    - 如果都没有，提示用户输入 PR 编号或确认使用 `git diff` 对比本地分支与 dev-1.0。
2. 有 PR 时使用 `gh pr diff`；否则使用 `git diff` 获取变更文件列表。
3. **审查范围**：
...
3. **CI 状态检查**：
   - 使用 `gh pr checks [PR编号]` 查询该 PR 的 CI 检查状态。
...
1. **获取评论**：使用 `gh api repos/{owner}/{repo}/pulls/{PR编号}/comments` 获取 PR 中的 review comments，并筛选 CodeRabbit 作者。
