...
> 📌 **数据源接入方式（R-5.2 决策 2026-08-01）**：受 iOS 公开 API 限制，Echo 无法自动/后台读取系统备忘录、语音备忘录或 iMessage。已批准路径：**Photos 全量读取（PhotoKit）+ Share Extension 用户主动分享**。备忘录/语音备忘录需用户通过分享按钮显式摄入。
...
| 数据源 | 接入方式 | AI 能力 | 状态 |
|--------|---------|---------|:---:|
| 📷 **相册图片** | PhotoKit 全量读取（自动） | SigLIP2 视觉语义理解，支持「那只穿红裙子的猫」这类自然语言搜索 | 🔶 |
| 🎬 **相册视频** | PhotoKit 全量读取（自动） | 帧级语义索引 + 语音内容全文检索 | 🔶 |
