...
5. HealthKit 健康上下文只请求 HRV read type。HealthKit 不向 App 披露读取授权的 granted/denied，`authorizationStatus(for:)` 仅表示分享/写入状态，`requestAuthorization` 的 success 仅表示授权流程完成。领域快照因此分为 `requestState = notRequested | requestCompleted | unsupported` 与 `dataState = samplesAvailable | noReadableSamples | unavailable`；不得向 UI 暴露伪造的 read-granted/read-denied。
