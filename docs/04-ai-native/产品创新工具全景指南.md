...
1. **统一 Embedding 引擎**：当前 Echo 使用 Qwen3-Embedding 处理文本、MobileCLIP2-S4 处理图片。引入 Gemini Embedding 2 后，可用单一模型处理所有模态，大幅简化架构。但在替换前必须验证端侧推理性能（目前主要依赖云端 API，需与 Google 洽谈端侧部署方案）
