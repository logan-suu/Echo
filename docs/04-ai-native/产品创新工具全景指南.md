...
Gemini Embedding 2 是对 Echo 当前视觉/文本向量（SigLIP2-B/32 候选 + multilingual-e5-small）的**战略替代方案**（研究参照）：

1. **统一 Embedding 引擎**：当前 Echo 使用 multilingual-e5-small 处理文本、SigLIP2-B/32（R-5.1 转换中）处理图片。引入 Gemini Embedding 2 后，可用单一模型处理所有模态，大幅简化架构。但在替换前必须验证端侧推理性能（目前主要依赖云端 API，需与 Google 洽谈端侧部署方案）
