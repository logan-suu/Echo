...
**视觉编码模型升级（替换 MobileCLIP2）** 与 **多模态检索引擎增强（US-SRC-010）**

### 融入方案

Gemini Embedding 2 是对 Echo 当前 MobileCLIP2-S4 向量的**战略替代方案**：

1. **统一 Embedding 引擎**：当前 Echo 使用 multilingual-e5-small 处理文本、MobileCLIP-B LT 处理图片。引入 Gemini Embedding 2 后，可用单一模型处理所有模态，大幅简化架构。但在替换前必须验证端侧推理性能（目前主要依赖云端 API，需与 Google 洽谈端侧部署方案）
...
4. **降级策略对齐**：如采用 Gemini Embedding 2 作为主力，需保留 MobileCLIP2-S4 作为本地离线降级方案，符合 US-RES-002（低电量）/US-RES-003（过热）的降级要求
