...
### 1.1 ⚠️ 使用边界：云端与联网能力（研究专用，生产禁用）

Echo 生产环境**绝对禁止**任何数据上传云端或依赖网络推理（红线 R-001「禁止任何数据上传云端」与 R-005「模型加载无网络下载」）。以下能力**仅允许在研究与预研分支使用**，**不得**进入 Echo 生产代码、默认 App 或发布产物：

| 能力 | 分类 | 依据 |
|------|------|------|
| **Firebase Vertex AI SDK**（云端 Gemini 模型调用） | 🔬 研究专用 / ❌ 生产禁用 | 违反 R-001 |
| **联网搜索增强**（network search，实时信息检索） | 🔬 研究专用 / ❌ 生产禁用 | 违反 R-001 |
| **云端回退**（cloud fallback，端侧失败时回退云端模型） | 🔬 研究专用 / ❌ 生产禁用 | 违反 R-001 与 R-005 |
| **端云协同模型个性化**（依赖云端超网络的方案，如 CHORD） | 🔬 研究专用 / ❌ 生产禁用 | 依赖云端协作，违反 R-001 |
| **云端 Embedding API**（如 Gemini Embedding 2 云端调用） | 🔬 研究专用 / ❌ 生产禁用 | 违反 R-001；仅端侧部署落地后才可能纳入评估 |

> **判定规则**：任何工具/方案只要依赖云端 API、联网搜索、联网模型分发或云端训练/超网络，即自动归类为「研究专用」，禁止进入生产。Echo 生产 AI 能力全部使用随 App 打包的端侧模型（Core ML + whisper.cpp）与 Apple 系统能力，所有处理在端侧完成。
...
**视觉编码模型升级（替换 SigLIP2 候选）** 与 **多模态检索引擎增强（US-SRC-010）**

### 融入方案

Gemini Embedding 2 是对 Echo 当前视觉/文本向量（SigLIP2-B/32 候选 + multilingual-e5-small）的**战略替代方案（研究参照）**。⚠️ **当前依赖云端 API，分类为研究专用（§1.1），在完成端侧部署前不得进入生产**：

1. **统一 Embedding 引擎**：当前 Echo 使用 multilingual-e5-small 处理文本、SigLIP2-B/32（R-5.1 转换中）处理图片。引入 Gemini Embedding 2 后，可用单一模型处理所有模态，大幅简化架构。但在替换前必须验证端侧推理性能（目前主要依赖云端 API，需与 Google 洽谈端侧部署方案；云端调用违反 R-001，仅研究分支可用）
...
4. **降级策略对齐**：如采用 Gemini Embedding 2 作为主力，需保留本地离线降级方案（当前无降级视觉模型，SigLIP2 转换中），符合 US-RES-002（低电量）/US-RES-003（过热）的降级要求
...
> ❌ **生产禁用（研究专用）**：本节描述的云端调用、联网搜索与云端回退全部违反红线 R-001（禁止任何数据上传云端）与 R-005（模型加载无网络下载），**仅允许在研究/预研分支使用**，不得进入 Echo 生产代码、默认 App 或发布产物。参见 §1.1。
...
**多模态输入处理** 与 **云端辅助增强（可选）** —— 以下为研究分支设想，**不作为生产方案**：

### 融入方案

虽然 Echo 的核心原则是 **本地优先**，但在某些场景下，云端辅助可提供更强的能力。**研究分支**中的集成设想需严格遵循隐私设计，且任何云端数据外发都违反 R-001，不得进入生产：

1. **可选联网搜索增强**：在 `SearchPipeline` 中增加一个可选步骤——如果用户明确开启“联网搜索”且查询需要实时信息（如“今天的重要新闻与我的记忆有什么关联？”），调用 Firebase Vertex AI 结合 Google Search 获取实时数据后，与本地检索结果融合。⚠️ 联网搜索违反 R-001，**仅研究分支评估**，生产禁用
2. **端云协同的模型调用**：对于 Echo 端侧模型无法处理的复杂任务（如长视频的深度语义理解），可回退到云端的 Gemini（需用户授权），用 `PrivacyCheckpoint` 记录每一次云端调用。⚠️ 云端回退违反 R-001/R-005，**仅研究分支评估**，生产禁用
3. **数据脱敏要求**：所有送往云端的数据必须经过匿名化/脱敏处理，原始记忆原文不得离开设备。⚠️ 即使脱敏，云端外传仍违反 R-001，生产一律禁止；生产场景必须由端侧模型（Core ML + whisper.cpp）承担
...
4. **云-端协作架构**：云端超网络负责分析和分发用户专属的量化策略，Echo 端侧只需应用该策略，无需上传任何记忆原文，符合隐私原则。⚠️ 该架构依赖云端协作，分类为研究专用（§1.1）；在端侧独立方案（如 SALT 的完全离线微调）落地前，不得进入生产
...
> ⚠️ **生产禁用注记（§1.1）**：Gemini Embedding 2（依赖云端 API）与 Firebase Vertex AI（云端调用/联网搜索/云端回退）在当前形态下为 **研究专用**，禁止进入 Echo 生产（违反 R-001/R-005）。表内“短期采纳/持续关注”仅表示研究优先级，不代表生产可用。云端与联网能力不得加入默认 App 或发布产物。
