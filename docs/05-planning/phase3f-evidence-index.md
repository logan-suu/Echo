...
- Cumulative test command / exit / passed count: `xcodebuild test ... -only-testing:EchoTests` — 883 tests / 113 suites, 0 failures, exit 0 (237s)
...
| AC # | Spec Summary | Test File | Implementation | Status |
| --- | --- | --- | --- | --- |
| US-ING-006 AC-1/2 | canonical+vector+FTS same transaction / compensation rollback | EchoTests/Phase3F/3F.4_CanonicalGenerationTests.swift (TransactionalCommit) | CanonicalMemoryRepositoryActor.commit + DatabaseManager.executeTransaction | ✅ |
...
- Cumulative: `xcodebuild test ... -only-testing:EchoTests` — 883 tests / 113 suites, 0 failures, exit 0 (237s)
