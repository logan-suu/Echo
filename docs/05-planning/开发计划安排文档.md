...
| R-3 | AI 运行时 Stub → 生产（E5 384d 独立索引 + SigLIP2 视觉 + Whisper ASR + OCR + 词法 + RRF） | ✅ 架构落地（推理接入排入 Phase 4：任务 4.21 SigLIP2 / 4.22 Whisper） | 2026-08-01 |
...
### 阶段3F：功能完成与生产集成（2026-08 启动，当前阶段）
...
| 3F.11 | Production E2E 与 Phase 4 准入门禁 | no-fixture E2E、Release 门禁全绿（阶段集成测试） | `EchoTests/Phase3F/Phase3FIntegrationTests.swift` |

**Phase 4 准入门禁**：`3F.11` 人类合并后才把 `current_phase` 从 `"3F"` 改为 `"4"`；在此之前 Phase 4 任何任务不得为 `ready` 或 `in_progress`。禁止在 `3F.11` 之前启动 Golden、性能、TestFlight、Release Candidate 或最终发布集成。
...
- Phase 4 保留 14 个任务（4.1..4.14），全部 `backlog` 且依赖 `3F.11`

**阶段交付**：默认 App 完成同意、真实来源、真实模型、规范存储、摄入、检索、反馈、编辑/删除、唤醒、翻译/创作、重启恢复闭环；`3F.11` 的 Release device/simulator、全套测试、覆盖率、隐私、制品、签名与 no-fixture E2E 全绿，v1 范围内 P0=0、P1=0。
