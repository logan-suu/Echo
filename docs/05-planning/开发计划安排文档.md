...
- composition-owned task reconstruction registry 按持久化 `taskType` 重建可继续任务；未知或过期类型 fail closed 并提供可恢复错误。
- Continue 从保存的准确 resume point 执行；Restart 先事务删除旧进度再从头创建任务；完成后立即清理进度。
- 取消、重启、失败与用户选择均走真实 TaskQueueActor/ProgressActor 路径和 hash-only 审计，不以 prompt fixture 证明生产恢复。
