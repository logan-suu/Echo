...
**生效日期**：2026-06-15（最后更新：2026-09-02）
...
- `3F.finalize` 最初解锁 14 个发布资格任务（4.1..4.14）；2026-08-31 人类批准全 App 平衡画布后新增任务族 `4.0`/`4.0a`/`4.0b`/`4.0c`；后续规格审查新增 `4.0d`~`4.0g`。2026-09-03 ADR-017 将过大的 `4.0e` 收敛为编辑/冲突，并新增来源删除 `4.0h`、引用/分享 `4.0i` 与叙事调度 `4.0j`，Phase 4 共 25 个任务。消费最终 UI 的下游任务按实际范围依赖对应生产边界，测试任务不得补写功能

**阶段交付**：默认 App 已完成同意、真实来源、真实模型、规范存储、摄入、检索、反馈、编辑/删除、唤醒与翻译/创作的基本闭环，并完成持久化进度检测与恢复提示展示。可执行的 Continue/Restart 重建闭环仍由 `4.0g` 交付。`3F.11` 证据证明 unsigned Release build、基线 coverage、隐私/合规扫描和 no-fixture E2E；它不宣称全局覆盖率 `>=95%`、受控签名/archive、最终 App Icon、Golden/性能或最终 RC 已完成，这些属于 Phase 4。
...
> Phase 4 以发布资格为主，不重复开发 Phase 3F 已完成的基本业务闭环。新增产品范围包括平衡画布 `4.0`~`4.0c`，以及规格审查确认尚未完成的生产闭环 `4.0d`~`4.0j`；其余任务负责验证、加固和发布。只有 `4.0`~`4.0j` 与 `4.1`~`4.9` 全部完成后才能生成唯一 RC。`task-status.json` 是任务状态、依赖与证据字段的唯一权威。
...
| 4.0d | 交互式唤醒卡生产闭环 | US-AWK-005 真实媒体、全离线音乐、下一张、MemoryFeeling、card→Focus 路由与审计 | 4.0a, 4.0b |
| 4.0e | 记忆编辑、重新向量化与持久冲突闭环 | MemoryUserEdit、成功后发布新表示、MemoryEditConflict 与 userLocked 同步边界 | 4.0b |
| 4.0f | 渐进式权限与首次体验生产闭环 | PIPL 先行、照片显式连接、唤醒权限按功能 opt-in、拒绝可继续 | 4.0c |
| 4.0g | 后台任务断点恢复生产闭环 | 任务重建注册表、继续/重新开始、重启恢复与真实审计 | 4.0c |
| 4.0h | 真实来源解析与 PhotoKit 删除补偿闭环 | 来源能力矩阵、真实媒体、PhotoKit 删除 saga、D-005 journal 重放 | 4.0e |
| 4.0i | 可验证引用、稳定导航与系统分享审计闭环 | source ID allow-list、NoSource、Detail 路由、结构化 sharePresented | 4.0h |
| 4.0j | 月度与年度叙事报告持久调度闭环 | period key、earliest-eligible、TaskQueue/Progress、实际来源审计 | 4.0i |
| 4.1 | Golden Dataset 与反馈排序质量验收 | 800+ 跨语言、200+ 反馈、100+ 术语用例及 Recall@10 报告 | 3F.11 |
| 4.2 | 全 Pipeline 生产端到端集成测试 | 扩展 no-fixture 正/负向、迁移与重启恢复矩阵，并覆盖跨 Surface 旅程 | 4.0a~4.0j |
| 4.3 | 性能基准与资源门禁验证 | P95、内存、CPU、energy、thermal 及 Discovery→Focus launch/hitch 原始证据 | 4.0a, 4.0b |
| 4.4 | 低电量、过热与模型加载恢复验证 | 降级、L1~L4、取消/恢复与 Task surface 运行时状态 | 4.0c, 4.0g |
| 4.5 | 数据删除与 ExcludedAssets 边界验收 | D-005 原子清除、PhotoKit saga、级联删除与恢复边界 | 4.0h |
| 4.6 | 覆盖率、静态分析与隐私门禁复核 | 含全部页面族及生产闭环新代码的全局覆盖率 >=95%、PrivacyCheckpoint 100%、静态禁令 | 4.0a~4.0j |
| 4.7 | 双语、本地化与无障碍验收 | 全 App 双语 100%、AX、Dynamic Type、对比度、family 回退与等价动作 | 4.0a~4.0j |
| 4.8 | 完成项目与发布文档更新 | 规格/架构/数据流/全 App UI 行为/发布文档与 deferred 证据互链 | 4.0a~4.0j |
| 4.9 | 发布合规、归档与签名验证 | 受控 signed archive/export、entitlements、SBOM、零外传 | 3F.11, 4.0f |
| 4.10 | 生成并批准 Release Candidate | 唯一 RC 身份与 Release Manager 批准；DEF-001/002 必须先处置 | 4.0~4.0j, 4.1~4.9 |
...
| 4.14 | Phase 4 集成测试：质量保障与最终发布验证 | Phase 1~4 累计测试、最终 RC 与人类发布批准 | 4.0~4.0j, 4.1~4.13 |
...
- **功能所有权**：4.0b 可调整 UI View、UI 值类型/薄 adapter、Focus contracts、fixtures 和测试，但不得新增数据库 schema、Core 写路径或第二份领域真相。现有生产边界缺失时必须显示不可用/失败，不得通过本任务的 fixture 成功态宣称 AC 完成；编辑/冲突由 `4.0e`、来源生命周期由 `4.0h`、引用/分享由 `4.0i`、报告调度由 `4.0j` 交付，4.2/4.5 只消费并验证。
...
##### 4.0e 记忆编辑、重新向量化与持久冲突闭环

`4.0e` 只交付 US-AWK-007。v1 描述为多行纯文本；标题、描述与规范化标签进入 `MemoryUserEdit` 关系，原始来源内容保持不变。有效检索文本按固定顺序组成，新表示准备成功后再发布并清理旧向量/FTS/缓存。冲突保存到 `MemoryEditConflict`，local/external/merge 决议持久更新 `userLocked`；同步中保存按 L4 阻止，自动同步跳过已锁定记忆。测试文件为 `EchoTests/Phase4/4.0e_FocusProductionActionsTests.swift`。

##### 4.0h 真实来源解析与 PhotoKit 删除补偿闭环

`4.0h` 交付 Focus 真实媒体/文本来源能力和 US-PRV-004 原始删除边界。只有当前可写、可解析的 PhotoKit photo/video 显示“同时删除原始文件”；Share Extension note/voice/thirdParty 不具备来源 App 删除权限。流程在请求系统删除前持久化既有 `MemoryDeletionJournal(.planned)`，成功后从 `.planned` 进入 Echo D-005 清理；系统拒绝/取消/失败保持 canonical 不变，恢复未决 intent 时先复验资产存在性，系统成功后的本地失败可重放且不写 ExcludedAssets。测试文件为 `EchoTests/Phase4/4.0h_FocusSourceLifecycleTests.swift`。

##### 4.0i 可验证引用、稳定导航与系统分享审计闭环

`4.0i` 要求生成协议显式返回 source MemoryID 并以本次输入集合校验，禁止 round-robin 绑定；未知/缺失/撤权来源为 `NoSource`。有效锚点经 UserPolicy 与 resolver 复验后进入稳定 Detail。`.creativeGeneration` 与 `.creationSharePresented` 分离，`sharePresented` 为结构化布尔字段；不记录 activityType、目标 App、用户完成状态或原文。测试文件为 `EchoTests/Phase4/4.0i_GroundedCitationShareTests.swift`。

##### 4.0j 月度与年度叙事报告持久调度闭环

`4.0j` 以持久周期键、`TaskQueueActor` 与 `ProgressActor` 实现 earliest-eligible 月报/年报。App 启动、前台或系统提供后台机会时执行最早未完成周期；每周期最多成功一次。只使用当前授权且生产可用的数据分区，不可用的人物/HealthKit 分区省略并标注。测试文件为 `EchoTests/Phase4/4.0j_NarrativeReportSchedulerTests.swift`。
...
Phase 3/3F 只交付了唤醒卡持久化、最小化通知和 response→detail 路由；`phase3f-story-matrix.md` 仍明确记录 US-AWK-005 的媒体/音乐、reaction persistence 与卡片交互为 Stub。4.0d 必须在 4.0a 的稳定 Discovery 卡片与 4.0b 的 Focus 路由基础上交付完整 AC，而不是把测试夹具或通知占位视为完成。2026-09-02 合理性复审由 ADR-016 管辖：

- **卡片内容**：真实本地文案；仅对可解析媒体显示 1~3 个真实缩略图。音乐默认从随包离线年份曲库生成建议；manifest 声明 supportedYears、每年 20 条元数据、fallback 和 provenance/license，不打包音频/封面/歌词/URL，并按 memoryId digest 稳定选择。用户显式选择“匹配此设备音乐”后才请求媒体库权限，且只读 `isCloudItem == false` 的本地曲目。禁止 MusicKit catalog/personal/recently-played 网络请求；denied/restricted/empty/failure 都回退随包库。
- **交互等价**：专用 AwakeningCard variant 支持左滑下一张、右滑记录感受，并提供等价可见按钮与 `accessibilityAction`；通用 Home/Search 卡不继承该手势。`next` 按稳定唤醒顺序前进，无后继项禁用；取消感受编辑器不产生写入或 `record` 审计。
- **感受数据**：领域层使用 `userFeelings` 集合，物理层使用以 memoryId 为外键的 `MemoryFeeling` 关系表。支持查看、编辑、删除、原 memory 级联删除和重启恢复；不创建 Memory/Representation，不进入搜索/翻译索引，删除后的个性化刷新遵循 US-AWK-005 AC-4。
- **来源导航**：4.0d 只负责从卡片发出稳定 memoryId/sourceLocator intent；4.0h 解析真实来源并处理资产失效、撤权与 PhotoKit 原始删除，4.0i 复验 source anchor 并进入 4.0b 已有的类型化 Echo Focus surface。不可解析来源诚实降级。
- **隐私与审计**：所有处理端侧完成；异步入口执行 PrivacyCheckpoint；`.cardInteraction` 仅在动作执行后记录 next/record/jump、hash-only card/memory digest 和布尔 `feelingAssociatedToSource`，不记感受原文或媒体库内容。
- **验收**：`EchoTests/Phase4/4.0d_InteractiveAwakeningCardTests.swift` 覆盖 AC-1~5、媒体库 denied/restricted/empty/failure 回退、无 MusicKit web-service 路径、事务故障、原 memory 级联删除、不可搜索/翻译索引、重启、双语、VoiceOver/等价动作与双设备无媒体 Live Simulator Review。
...
- 生产状态不可用时显示诚实不可用/错误，不得回退 fixture；同意/拒绝保持同层可见和同等醒目。
- 六个 Task surface 的 legacy state/action/journey 必须在本任务内补齐或迁移到 v1 schema-compatible 结构；保持既有稳定 ID，所有非终态步骤必须解析到真实 action contract。
- 本任务不修复权限请求时机或任务重建：前者由 `4.0f`、后者由 `4.0g` 交付，避免在视觉切片中夹带领域副作用。
- 测试文件：`EchoTests/Phase4/4.0c_TaskBalancedCanvasTests.swift`。

##### 4.0f 渐进式权限门禁

- PIPL 同意持久化成功是受保护请求的硬门禁；失败停留并重试。PhotoKit 仅由 `Connect Photos` 请求，`Not Now` 零系统调用，limited/full/denied/restricted 均有诚实路径。
- 通知是独立投递 opt-in，不由三个唤醒开关隐式请求或统一门控 App 内卡片。geofence 先 When In Use，再由第二个明确动作请求 Always；anniversary 不请求系统权限。
- HealthKit 仅请求 HRV read scope；UI/领域契约只表达 notRequested/requestCompleted/unsupported 与 samplesAvailable/noReadableSamples，禁止伪造 read granted/denied。
- 通过 `AwakeningPreferenceActor` + SQLite 持久化三个唤醒偏好、通知投递意图与 HealthKit request lifecycle；系统授权状态不入库，重启恢复不得触发 prompt。
- 所有 adapter 等待系统回调或复读最终快照；打开 App/设置页、拒绝、跳过、重复进入与重启均不自动请求。
- 测试文件：`EchoTests/Phase4/4.0f_ProgressivePermissionFlowTests.swift`。

##### 4.0g 真实断点恢复门禁

- composition-owned typed allow-list registry 按精确 `taskId` + raw `taskType` 重建任务；未知/过期记录必须可见、fail closed 为 L2 并保留，不得被 enum 解码静默丢弃。
- `resumeData` 版本化、有大小上限/完整性校验，不存授权快照或原文；launcher 在恢复时按当前 UserPolicy/真实来源重做 PrivacyCheckpoint。
- Continue 从保存的准确 resume point 入队且不得先覆盖 checkpoint。Restart 先校验 launcher/授权，再在 SQLite 事务中替换为 index 0 可重试记录，然后单独入队；入队失败保留记录。
- 多条同 taskType 记录逐条呈现；当前会话 running/queued/paused taskId 不得触发恢复 prompt；同 taskId 重复点击幂等且不得双重入队。
- Pause 保留同一 in-memory job；Cancel 在协作式终止和最后 checkpoint 持久化后发布终态。已入队任务成功/最终不可继续失败清理进度；重建 L2、暂停、取消保留。
- 用户选择和真实入队结果走 TaskQueueActor/ProgressActor 路径及结构化非原文审计，不以 prompt fixture 或仅 UI 状态翻转证明生产恢复。
- 测试文件：`EchoTests/Phase4/4.0g_TaskResumeProductionTests.swift`。
