...
- `3F.finalize` 最初解锁 14 个发布资格任务（4.1..4.14）；2026-08-31 人类批准全 App 平衡画布后新增任务族 `4.0`/`4.0a`/`4.0b`/`4.0c`；2026-09-01 两轮规格审查分别新增 US-AWK-005 生产闭环 `4.0d` 与 Focus 生产动作/来源闭环 `4.0e`；2026-09-02 Task 规格审查新增渐进式权限 `4.0f` 与真实断点恢复 `4.0g`，Phase 4 共 22 个任务。`4.0` 只负责共享基础与 AppShell；三个页面族任务完成表现层，`4.0d`~`4.0g` 补齐不能由视觉任务或测试任务替代的生产行为。消费最终 UI 的下游任务按实际范围依赖对应任务，不再被单个巨型任务阻塞
...
> Phase 4 以发布资格为主，不重复开发 Phase 3F 已完成的基本业务闭环。新增产品范围包括人类批准的平衡画布任务族 `4.0`/`4.0a`/`4.0b`/`4.0c`，以及规格审查确认尚未完成的 US-AWK-005 生产闭环 `4.0d`、Focus 生产动作/来源闭环 `4.0e`、渐进式权限 `4.0f` 和真实断点恢复 `4.0g`；其余任务仍负责验证、加固和发布。只有 `4.0`~`4.0g` 与 `4.1`~`4.9` 全部完成后才能生成唯一 RC。`task-status.json` 是任务状态、依赖与证据字段的唯一权威。
...
| 4.2 | 全 Pipeline 生产端到端集成测试 | 扩展 no-fixture 正/负向、迁移与重启恢复矩阵，并覆盖跨 Surface 旅程 | 4.0a, 4.0b, 4.0c, 4.0d, 4.0e, 4.0f, 4.0g |
...
| 4.6 | 覆盖率、静态分析与隐私门禁复核 | 含全部页面族及生产闭环新代码的全局覆盖率 >=95%、PrivacyCheckpoint 100%、静态禁令 | 4.0a, 4.0b, 4.0c, 4.0d, 4.0e, 4.0f, 4.0g |
| 4.7 | 双语、本地化与无障碍验收 | 全 App 双语 100%、AX、Dynamic Type、对比度、family 回退与等价动作 | 4.0a, 4.0b, 4.0c, 4.0d, 4.0e, 4.0f, 4.0g |
| 4.8 | 完成项目与发布文档更新 | 规格/架构/数据流/全 App UI 行为/发布文档与 deferred 证据互链 | 4.0a, 4.0b, 4.0c, 4.0d, 4.0e, 4.0f, 4.0g |
| 4.9 | 发布合规、归档与签名验证 | 受控 signed archive/export、entitlements、SBOM、零外传 | 3F.11, 4.0f |
| 4.10 | 生成并批准 Release Candidate | 唯一 RC 身份与 Release Manager 批准；DEF-001/002 必须先处置 | 4.0/4.0a/4.0b/4.0c/4.0d/4.0e/4.0f/4.0g, 4.1~4.9 |
...
| 4.14 | Phase 4 集成测试：质量保障与最终发布验证 | Phase 1~4 累计测试、最终 RC 与人类发布批准 | 4.0/4.0a/4.0b/4.0c/4.0d/4.0e/4.0f/4.0g, 4.1~4.13 |
...
- **来源导航**：4.0d 只负责从卡片经稳定 memoryId/sourceLocator 进入 4.0b 已有的类型化 Echo Focus surface；不可解析来源诚实降级。Focus 内真实 source anchor、原来源删除和资产失效/撤权边界仍由 4.0e 唯一交付。
