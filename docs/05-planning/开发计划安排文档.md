...
- `3F.finalize` 最初解锁 14 个发布资格任务（4.1..4.14）；2026-08-31 人类批准全 App 平衡画布后新增任务族 `4.0`/`4.0a`/`4.0b`/`4.0c`，Phase 4 共 18 个任务。`4.0` 只负责共享基础与 AppShell；完成后 Discovery、Focus、Task 三个页面族任务可并行。消费最终 UI 的下游任务按实际范围依赖对应页面族，不再被单个巨型任务阻塞
...
> Phase 4 以发布资格为主，不重复开发 Phase 3F 已完成的基本业务闭环。唯一新增产品范围是人类批准的平衡画布任务族 `4.0`/`4.0a`/`4.0b`/`4.0c`；其余任务仍负责验证、加固和发布。只有该任务族与 `4.1`~`4.9` 全部完成后才能生成唯一 RC。`task-status.json` 是任务状态、依赖与证据字段的唯一权威。
...
| 任务ID | 任务名称 | 主要产出 | 当前状态 | 依赖 |
| ------ | -------- | -------- | -------- | ---- |
| 4.0 | 平衡画布设计系统与 AppShell 基础 | DesignProfile、共享 token/component、Apple 原生 AppShell | ready | 3F.11 |
| 4.0a | Discovery 平衡画布：Home 与 Search 真实数据 | Home 0/1~19/20+、Search <6/>=6、adaptive masonry 与单列回退 | backlog | 4.0 |
| 4.0b | Focus 平衡画布：Detail、Creation 与 Translation | 单列主内容、引用卡片、grouped metadata，同源非 masonry 表达 | backlog | 4.0 |
| 4.0c | Task 平衡画布：设置、引导与运行状态页面 | 六个 Task 功能域统一 grouped container、状态与动作层级 | backlog | 4.0 |
| 4.1 | Golden Dataset 与反馈排序质量验收 | 800+ 跨语言、200+ 反馈、100+ 术语用例及 Recall@10 报告 | ready | 3F.11 |
| 4.2 | 全 Pipeline 生产端到端集成测试 | 扩展 no-fixture 正/负向、迁移与重启恢复矩阵，并覆盖跨 Surface 旅程 | backlog | 4.0a, 4.0b, 4.0c |
| 4.3 | 性能基准与资源门禁验证 | P95、内存、CPU、energy、thermal 及 Discovery→Focus launch/hitch 原始证据 | backlog | 4.0a, 4.0b |
| 4.4 | 低电量、过热与模型加载恢复验证 | 降级、L1~L4、取消/恢复与 Task surface 运行时状态 | backlog | 4.0c |
| 4.5 | 数据删除与 ExcludedAssets 边界验收 | D-005 原子清除、级联删除与恢复边界 | ready | 3F.11 |
| 4.6 | 覆盖率、静态分析与隐私门禁复核 | 含全部页面族新代码的全局覆盖率 >=95%、PrivacyCheckpoint 100%、静态禁令 | backlog | 4.0a, 4.0b, 4.0c |
| 4.7 | 双语、本地化与无障碍验收 | 全 App 双语 100%、AX、Dynamic Type、对比度与 family 回退 | backlog | 4.0a, 4.0b, 4.0c |
| 4.8 | 完成项目与发布文档更新 | 规格/架构/数据流/全 App UI 行为/发布文档与 deferred 证据互链 | backlog | 4.0a, 4.0b, 4.0c |
| 4.9 | 发布合规、归档与签名验证 | 受控 signed archive/export、entitlements、SBOM、零外传 | ready | 3F.11 |
| 4.10 | 生成并批准 Release Candidate | 唯一 RC 身份与 Release Manager 批准；DEF-001/002 必须先处置 | backlog | 4.0/4.0a/4.0b/4.0c, 4.1~4.9 |
| 4.11 | TestFlight 内测（内部 50 人） | 同一 RC 的 TestFlight 分发与结构化反馈 | backlog | 4.10 |
| 4.12 | 修复 TestFlight 新发现 P0/P1 缺陷 | 可复现修复与 P0=0、P1=0 | backlog | 4.11 |
| 4.13 | Release Candidate 全量回归 | 最新 RC 累计回归、门禁重跑与双设备 E2E | backlog | 4.12 |
| 4.14 | Phase 4 集成测试：质量保障与最终发布验证 | Phase 1~4 累计测试、最终 RC 与人类发布批准 | backlog | 4.0/4.0a/4.0b/4.0c, 4.1~4.13 |
...
- **4.0 Foundation/AppShell**：DesignProfile schema/instance、共享 token/component 与 Apple 原生 AppShell；AppShell 是承载三类 Surface 的宿主而非 Task surface；本任务不修改具体功能页或领域行为。
...
- **0 条记忆**：品牌空态；根据真实授权/来源状态引导相册授权或系统分享导入，不展示伪造回忆、主题或假进度。
- **1~19 条记忆**：Ask Echo、可执行的真实建议问题和最近导入；不强行生成主题集合。
...
- **Search 一致性**：少于 6 个可展示结果或长文本占多数时保持相关度优先的单列；6 个及以上适合扫描的媒体/短摘要结果默认 adaptive masonry。布局不得改变 SearchPipeline 相关度顺序、partial/low-confidence 语义或反馈 `memoryId` 绑定。
- **运行时状态**：后台进度只来自 ProgressActor/TaskQueueActor 且无活跃任务自动隐藏；离线、低电量、过热、模型/索引不可用均来自真实运行时状态。
- **真实性与隐私**：默认生产路径禁止 fixture、Preview、静态示例和无法执行的建议问题；卡片保留来源身份与跳转目标，删除或权限撤回后不得展示陈旧内容；所有处理保持端侧，审计只写哈希与必要元数据。
- **验收**：测试文件为 `EchoTests/Phase4/4.0a_DiscoveryBalancedCanvasTests.swift`；覆盖 Home 0 / 1~19 / 20+、Search <6 / >=6、rich-data 双列、低数据/AX 单列回退、稳定 reading order 与无 fixture。
