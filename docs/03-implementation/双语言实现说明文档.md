...
- `userFeelings.text` 按用户输入原文存储，不自动翻译、不改写，也不写入 Memory/Representation/FTS/向量索引；因此不得被用户搜索或计入跨语言 Recall。
- “记录感受”、查看、编辑、删除、下一张及音乐授权/降级等 UI 文案必须来自 zh-Hans/en-US String Catalog；用户输入内容不因 UI 语言切换而变化。
