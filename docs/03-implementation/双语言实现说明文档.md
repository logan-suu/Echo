...
flowchart TB
    ...
    subgraph Retrieve["🔍 检索路径 (跨语言语义匹配)"]
        ...
        Rerank["GTE Cross-Encoder<br/>跨语言精排"]
...
- **文本模型**：multilingual-e5-small INT4，继承 100+ 语言对齐能力，768 维输出通过 384d 嵌入（纯文本检索用，跨模态需对齐层）。模型随 App 安装包分发，无网络下载。
- **视觉模型**：MobileCLIP-B LT，768 维输出与文本模型共享同一语义空间。低电量/过热时可降级为 MobileCLIP2-S3（预留）（同样保持 CLIP 空间兼容）。
- **对齐验证**：每次模型更新后，必须通过跨语言召回率基准测试（中英互检 Recall@10 ≥ 85%）。
- **降级兼容**：MobileCLIP2-S3（预留） 位于同一 CLIP 空间（仅为 S4 的同架构缩小型），确保设备过热/低电量时跨语言能力不退化。
...
sequenceDiagram
    ...
    participant Embed as GTE Embedder
    ...
    Embed->>Embed: multilingual-e5-small → 768-dim vector
    ...
    Retriever->>Retriever: GTE Cross-Encoder rerank (cross-lingual)
...
- **模型**：multilingual-e5-small Cross-Encoder INT4，与 Bi-Encoder 共享语义空间。
