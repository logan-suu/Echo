...
flowchart TB
    subgraph Ingest["📥 摄入路径 (仅外部数据源)"]
        ...
        Embed["Qwen3-Embedding-0.6B<br/>MRL 768d 向量"]
...
- **文本模型**：Qwen3-Embedding-0.6B INT4，继承 GTE-Qwen2 双语对齐能力，768 维输出通过 Matryoshka 截断与视觉模型对齐。模型随 App 安装包分发，无网络下载。
- **视觉模型**：MobileCLIP2-S4，768 维输出与文本模型共享同一语义空间。低电量/过热时可降级为 MobileCLIP2-S3（同样保持 CLIP 空间兼容）。
- **对齐验证**：每次模型更新后，必须通过跨语言召回率基准测试（中英互检 Recall@10 ≥ 85%）。
- **降级兼容**：MobileCLIP2-S3 位于同一 CLIP 空间（仅为 S4 的同架构缩小型），确保设备过热/低电量时跨语言能力不退化。
...
sequenceDiagram
    ...
    Embed->>Embed: Qwen3-Embedding-0.6B → 768-dim vector
...
- **模型**：Qwen3-Embedding-0.6B Cross-Encoder INT4，与 Bi-Encoder 共享语义空间。
