...
> **对应 Codex skills**：`$ui-bootstrap-build-echo`、`$init-session-echo`（UI 分支）、`$next-task-echo`（UI 分支）
...
- **3F.10 UI 验证（2026-08-12）**：新增 `EchoUITests/LocalizationAccessibilityUITests`（语言 Picker/低电量开关）+ `EchoUITests/DegradationUITests`（lowPower/thermal/modelDegraded 双语言旅程，含 zh-Hans）；既有 UI 套件经 `-ui-language en-US` 保证确定性；42 UI 测试 0 失败
