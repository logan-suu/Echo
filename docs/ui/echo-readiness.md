...
- **3F.10 交付（2026-08-12）**：`Localizable.xcstrings`（zh-Hans/en-US，336 keys）成为 UI 文案唯一来源；`LanguageCenter` 统一 App 语言（US-DIS-001）；`EchoStrings.tr`/`.environment(\.locale)` 双通道解析；`Scripts/validate_localization.py`、`validate_static_bans.py`、`validate_accessibility_contracts.py` 为 3F.10 静态门禁；双设备 Live Sim Review 通过（run manifest `visualMediaCaptured: false`）
