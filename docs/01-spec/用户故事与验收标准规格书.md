...
12. 首次体验遵循渐进式权限：PIPL 同意先于任何受保护数据请求；相册权限只在用户明确选择连接照片资料库后请求。通知、位置与 HealthKit 不在首次启动中串行弹出，只在用户进入相应唤醒功能并明确启用时按需请求。拒绝或跳过任一可选权限不得阻止进入 App，也不得循环催促；系统设置入口仅在用户主动请求恢复授权时出现。
...
- 验收标准
  ...
  3. 权限请求文案清晰说明用途，支持“仅授权部分相册”。PIPL 同意完成后，只有当用户明确选择连接照片资料库时才请求 PhotoKit 权限；拒绝或跳过后仍可通过 Share Extension 使用备忘录/语音备忘录，不得在首次启动流程中反复请求。
...
> 📌 **实现状态（3F.6，2026-08-11）**：search 契约已实现——`CrossAppIntentParser` 解析 health+memory 与 location+photo 意图；`CrossAppFusionEngine` 逐源 Privacy Gate（未授权源不调用 provider）、按解析的时间窗口对齐结果、保留来源标签、审计记录 `.crossAppSearch`（含实际授权源列表）。**3F.8（2026-08-11）**：live HealthKit provider 接入——`HealthKitSystemProvider` 符合 `CrossAppSourceProvider`（sourceType="health"），向 health+memory 融合提供授权范围内时间窗内最小化样本，保留来源身份；denied HealthKit 来源不查询（ADR-012 决策-2/4）。
...
- 验收标准
  ...
  1. 由系统后台调度在每日 **earliest-eligible / best-effort 窗口**检查“往年今日”+ 用户自定义纪念日；不得承诺或在 UI 中显示精确 9:00 执行。用户进入前台时可补做当日尚未完成的检查，同一自然日只完成一次有效检查。
...
> 📌 **实现状态（3F.8，2026-08-11）**：`HealthKitSystemProvider` 接入生产——HRV 情绪推断（US-AWK-003 AC-1）且符合 3F.6 `CrossAppSourceProvider`（sourceType="health"）供 US-SRC-010 fusion；权限感知（denied 不查询）；数据最小化（仅最小化时序样本）。ADR-012 决策-2/4 管辖。
