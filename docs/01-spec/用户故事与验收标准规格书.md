...
2. 0 条记忆显示诚实空态；1~19 条显示 Ask Echo、真实可执行建议与最近导入；20 条及以上进入 rich-data 状态。任何状态均禁止 fixture、Preview、静态示例或伪造进度补位。
3. rich-data 状态在默认 Dynamic Type、足够内容宽度且单个区块至少有 6 张真实可展示卡片时，默认使用双列 adaptive masonry；未满足条件时回退单列。
4. Search 是 `discovery` surface；至少 6 个适合视觉扫描的媒体/短摘要结果时可使用同一平衡画布，但必须保持 SearchPipeline 的相关度语义顺序。长文本占多数、低结果量或无障碍条件下保持单列。
5. 卡片高度只来自真实媒体 aspect ratio 或文本摘要长度，不使用随机高度；每张卡片必须保留稳定 `memoryId`、来源身份、时间/地点 metadata 与 Focus 详情跳转。
