...
- 验收标准
  ...
  2. 保存时基于固定顺序的有效文本（用户标题、用户描述、规范化标签、原 canonical 文本）生成新表示与向量，成功后原子发布新的 `MemoryUserEdit`/FTS/representation 绑定并删除旧向量及缓存；任一步失败保留旧可服务版本。更新 `userEdited=true`。首次覆盖时间戳时将原值写入 `originalTimestamp`，后续覆盖不得改写该备份。
