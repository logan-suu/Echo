...
10. Phase 3/3F 证据中仍为 Partial 的 Focus 生产写入与来源动作由独立任务 `4.0e` 交付：编辑后事务持久化与重新向量化、`userLocked` 冲突决议及同步跳过、原始来源删除、真实 video/voice 媒体解析、source anchor 跳转和 `sharePresented` hash-only 审计。`4.0b` 只提供诚实表现，`4.2` 只验证闭环；两者都不得替代 `4.0e` 的生产实现。
...
- 验收标准
  ...
  6. 审计记录 `.creativeGeneration`，含 `templateType`、`sourceMemoryCount`、`exportFormat`、`sharePresented`；不得记录或推断目标 App、用户是否完成保存或生成的内容原文。
...
- 验收标准
  ...
  6. 审计记录 `.narrativeReportGenerated`，含 `periodType`、`dataSourcesUsed`、`sharePresented`；不推断用户最终选择的目标 App 或保存结果。
...
- 验收标准
  ...
  3. 卡片中的媒体/文案/语音动作通过稳定 `memoryId` 与当前授权下可解析的 `sourceLocator` 进入 Echo 已有的类型化 Focus surface：照片/视频 → 媒体详情，备忘录文案 → 原文阅读，语音 → 本地播放。来源失效或撤权时显示诚实不可用状态，不回退到 fixture/bundled sample。`4.0d` 负责 card → Focus 的 intent 与路由；`4.0e` 负责 Focus 内部真实 source anchor、原始来源删除等生产边界。
