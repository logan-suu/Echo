...
**最近规格审查**：2026-09-02（4.0c Task 表现边界、渐进式权限与真实断点恢复）
...
- 验收标准
  ...
  4. 若用户已在编辑界面而外部数据源发生变更，系统不应自动覆盖，而应将外部版本摘要与检测时间持久化到 `MemoryEditConflict`，并在当前或下次打开编辑时显示“**内容冲突**：外部数据源已被修改，您的编辑尚未保存。请选择：保留您的编辑 / 使用外部新版本 / 查看差异并合并”。需提供差异对比 UI。选择“保留您的编辑”或完成手动合并后清除 conflict、写 `userLocked=true`，后续自动同步跳过该记忆，直到用户显式“重新同步”或恢复外部版本；选择外部版本时替换 canonical/representation，清除用户编辑与 conflict，并写 `userLocked=false`。
