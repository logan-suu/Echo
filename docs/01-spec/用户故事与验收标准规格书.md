...
- 验收标准
  ...
  1. 用户明确启用健康上下文后，仅请求读取 HRV 所需的最小 HealthKit 类型。授权请求完成后记录 app-level `requestCompleted`，不得显示 HealthKit read `granted/denied`；仅当查询实际返回可读 HRV 样本时使用 HealthKit 推断情绪状态。设备不支持 HealthKit 时显示 `unsupported`，不发起请求。
